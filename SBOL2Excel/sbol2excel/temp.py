...
def callURI(URI, cwd):
    """
    To convert your own SBOL file into and Excel spreadsheet, insert your own
    URI as the first parameter. The 'cwd' parameter has been given to you at
    the top.
    """
    new_xml = 'myFile.xml'
    imported_xml = requests.get(URI)
    output_path_new_xml = os.path.join(cwd, 'SBOL2Excel', 'tests',
                                            'test_files',
                                            new_xml)
